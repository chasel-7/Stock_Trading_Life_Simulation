...
- Modify: `src/scenes/BootScene.ts`（跳转目标改为角色选择）
- Modify: `src/managers/GameManager.ts`（支持动态角色）

- [ ] **Step 1: 创建 src/scenes/CharacterSelectScene.ts**

```typescript
import Phaser from 'phaser';
import { CHARACTERS, CHARACTER_ORDER, getCharacter } from '../data/characters';
...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const detailBg = this.add.rectangle(width / 2, detailY + 40, width - 40, 80, 0x16213e, 1)
      .setStrokeStyle(1, 0x333366);
...
- [ ] **Step 2: 更新 gameConfig.ts 注册 CharacterSelectScene**

在 `src/config/gameConfig.ts` 的 scene 数组中，在 BootScene 之后添加 CharacterSelectScene：
...
- [ ] **Step 3: 修改 BootScene 跳转到角色选择**

将 `src/scenes/BootScene.ts` 中的「新游戏」按钮跳转目标从 `'PreMarketScene'` 改为 `'CharacterSelectScene'`：
...
- [ ] **Step 4: 修改 GameManager 支持动态角色ID**

在 `src/managers/GameManager.ts` 中：
...
export class GameManager {
  ...
  constructor(characterId: string = 'programmer') {
...
在 `CharacterSelectScene.confirmSelection()` 中调用 `reset`：
...
Expected: 新游戏→角色选择界面→4个角色卡片可点击→底部显示属性详情→确认后进入盘前。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat: add CharacterSelectScene with 4 characters and dynamic GameManager"
```
