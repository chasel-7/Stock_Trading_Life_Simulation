...
**Files:**
- Create: `src/utils/posterGenerator.ts`
- Modify: `src/scenes/ReviewScene.ts`（第5步：分享海报）

- [ ] **Step 1: 创建 src/utils/posterGenerator.ts**

利用Phaser内置的canvas截图能力生成分享海报：
...
export function generatePoster(scene: Phaser.Scene, data: PosterData): void {
  ...
  // 四维评分
  const scoreLabels = [
    `收益力 ${data.scores.returnScore}`,
    `风控力 ${data.scores.riskControl}`,
    `投资智慧 ${data.scores.investWisdom}`,
    `生存能力 ${data.scores.survivalSkill}`,
  ];
...
- [ ] **Step 2: 在 ReviewScene 第4步之后添加分享按钮**

在 ReviewScene 的 showRadarAndTitle() 方法末尾追加：

```typescript
import { generatePoster, PosterData } from '../utils/posterGenerator';

// 在"再来一局"按钮旁边添加"分享海报"按钮
const shareBg = this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT - 110, GAME_WIDTH - 40, 44, 0xf39c12, 1)
  .setInteractive({ useHandCursor: true });
this.add.text(GAME_WIDTH / 2, GAME_HEIGHT - 110, '📤 生成分享海报', {
  fontSize: '16px', color: '#fff', fontFamily: 'sans-serif',
}).setOrigin(0.5);
...
Expected: 复盘最后一步出现"生成分享海报"按钮，点击后截图并弹出分享/下载。

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "feat: add share poster generation with Web Share API fallback"
```
